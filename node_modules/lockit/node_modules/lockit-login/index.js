...
Login.prototype.postLogin = function(req, res, next) {
  ...
  // check if login is a name or an mobile

  // regexp from https://github.com/angular/angular.js/blob/master/src/ng/directive/input.js#L4
  var MOBILE_REGEXP = /^1\d{10}$/;
  var query = login.match(MOBILE_REGEXP)?'mobile':'name';

  // find user in db
  adapter.find(query, login, function(err, user) {
    if (err) return next(err);

    // no user or user mobile isn't verified yet -> render error message
    if (!user ) {
      error = '用户不存在';
    ...
    pwd.hash(password, user.salt, function(err, hash) {
      ...
      adapter.update(user, function(err, user) {
        ...
        // create session and save the name and mobile
        req.session.name = user.name;
        req.session.mobile = user.mobile;
        req.session.contact = (typeof (user.contact) === 'undefined')? "" : user.contact;
